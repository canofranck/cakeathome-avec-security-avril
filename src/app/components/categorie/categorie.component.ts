...
export class CategorieComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.recetteService.findAllRecettes().subscribe((data) => {
      ...
      this.nombresRecettesParCategorie = this.compterRecettesParCategorie(
        this.recettes
      ); // Comptage des recettes par catégorie
    });
  }
  ...
  compterRecettesParCategorie(recettes: any[]): any[] {
    ...
    const resultats = recettes.reduce((acc, recette) => {
      ...
      if (categorie in acc) {
        // Si la catégorie a déjà été rencontrée, on incrémente le nombre de recettes associé
        acc[categorie]++;
      } else {
        // Sinon, on ajoute une nouvelle entrée avec un compteur initialisé à 1
        acc[categorie] = 1;
      }
    ...
    return Object.entries(resultats).map(([categorie, nombre]) => ({
      categorie,
      nombre,
    }));
  }
