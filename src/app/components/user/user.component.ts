...
export class UserComponent implements OnInit {
  ...
  public declare refreshing: boolean;
  public declare users: User[];
  private subscription: Subscription[] = [];
  public declare selectedUser: User | null;
  public declare fileName: string;
  public declare profileImage: File;
  public editUser = new User();
  public declare currentUsername: string;
        ...
        (data: User[]) => {
          ...
          if (showNotification) {
            this.sendNotification(
              ...
              `${data.length} Utilisateur(s) chargés avec succés`
            );
          }
        },
    ...
    this.subscription.push(
      this.userService.addUser(formData).subscribe(
        (data: User) => {
          ...
          this.sendNotification(
            NotificationType.SUCCESS,
            `${data.firstname} ${data.lastname} a été ajouté avec succes`
          );
        },
        (errorResponse: HttpErrorResponse) => {
          this.sendNotification(
  ...
  public searchUsers(searchTerm: string): void {
    ...
    for (const user of this.userService.getUsersFromLocalCache()) {
      if (
        user.firstname.toLocaleLowerCase().indexOf(searchTerm.toLowerCase()) !==
          -1 ||
        user.lastname.toLocaleLowerCase().indexOf(searchTerm.toLowerCase()) !==
          -1 ||
        user.username.toLocaleLowerCase().indexOf(searchTerm.toLowerCase()) !==
          -1 ||
    ...
    this.subscription.push(
      this.userService.updateUser(formData).subscribe(
        (data: User) => {
          ...
          this.sendNotification(
            NotificationType.SUCCESS,
            `${data.firstname} ${data.lastname} a été édité avec succes`
          );
        },
        (errorResponse: HttpErrorResponse) => {
          this.sendNotification(
    ...
    this.subscription.push(
      this.userService.deleteUser(userId).subscribe(
        (data: CustomHttpResponse) => {
          this.sendNotification(
            NotificationType.SUCCESS,
            `L'utilisateur a bien été supprimé avec succes`
          );
          this.getUsers(true);
        },
        (errorResponse: HttpErrorResponse) => {
  ...
  onLogout() {
    ...
    this.sendNotification(
      NotificationType.SUCCESS,
      'Vous vous êtes bien déconnecté avec succes'
    );
  }
    ...
    this.subscription.push(
      this.userService.resetPassword(emailAdresse).subscribe(
        (data: CustomHttpResponse) => {
          this.sendNotification(
            NotificationType.SUCCESS,
            `Le password a bien été réinitialisé`
          );
          this.refreshing = false;
        },
        (errorResponse: HttpErrorResponse) => {
